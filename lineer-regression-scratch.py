...
class LineerRegression():
    ...
    def fit(self, X, y, learning_rate=0.01):
        ...
        for x_m, y_m in zip(X, y):
            ...
            hypothesis_x = self.hypothesis(x_m, self.THETA)

            loss = self.loss_function(hypothesis_x, y_m)

            THETA = self.update_parameters(self.THETA, hypothesis_x, x_m, y_m, self.learning_rate)
    ...
    def predict(self, X_test):
        ...
        xzeros = np.ones([X_test.shape[0], 1], dtype=X_test.dtype)
...
def main():
    X, y = make_regression(n_samples = 250, n_features=3, noise=20, random_state=0, bias=50)
